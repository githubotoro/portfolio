...
export const Tagline = ({ props }: { props: string }) => {
	...
	const BLURRY = clsx("blur-[0.1rem]");

	return (
		<h1
			id="tagline"
			className={clsx(
				"font-500 rounded-2xl w-full max-w-4xl text-isSystemLightTertiary text-[2.1rem] sm:text-[3.6rem] md:text-[5.2rem] flex flex-col space-y-1 md:space-y-3 leading-none font-serif text-center bg-[url('/assets/gifs/explosion.gif')]  bg-cover bg-no-repeat overflow-hidden relative",
				props
			)}
		>
			<div className="w-full h-full bg-isBlack bg-opacity-20 backdrop-blur-sm py-8 md:py-16 ">
				<div className="flex flex-row items-center place-content-center ">
					...
					{/* <span
					className={clsx(
						"relative overflow-hidden mx-2 w-[4.8rem] h-[2.4rem] md:w-[9.6rem] md:h-[4.8rem] md:mx-5 rounded-lg md:rounded-2xl hover:blur-none hover:scale-[1.5] hover:rotate-6 group",
						ANIMATE
					)}
				>
					<Image
